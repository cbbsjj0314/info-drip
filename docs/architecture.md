...
핵심은 PDF에서 궁금한 부분을 선택하고, 그 selected text를 중심으로 설명, 용어 정리, 질문/답변, 퀴즈, 다시 풀기 흐름을 이어 가는 것이다.
...
`study-records`는 backend/API capability이며 current iPad usage는 `저장된 문장`, `용어 모음`, saved sentence detail, replay/detail sheets 같은 saved result lookup flow를 통해 이루어진다.

별도의 `학습 기록` toolbar entry나 `DocumentStudyRecordSheet`를 current primary UI로 설명하지 않는다.
