...
InfoDrip은 개인용 local-first iPad PDF 읽기 보조 앱이다.

핵심은 PDF에서 궁금한 부분을 선택하고, 해당 selected text를 중심으로 설명, 용어 정리, 질문/답변, 퀴즈, 다시 풀기 흐름을 이어 가는 것이다.

InfoDrip은 범용 PDF 챗봇, public SaaS, full RAG system, OCR system이 아니다.

Primary MVP client는 SwiftUI와 PDFKit 기반 iPad 앱이다.

Backend는 FastAPI와 SQLite를 사용하며, LLM 호출은 OpenAI-compatible provider boundary 뒤에서 처리한다.

## 2. MVP workflow

현재 MVP workflow는 다음 흐름을 중심으로 한다.

1. iPad 앱에서 PDF를 import한다.
2. PDFKit reader에서 PDF를 읽고 텍스트를 선택한다.
...
5. Backend는 selected text와 주변 context를 사용해 LLM provider를 호출하고, 응답을 검증한 뒤 SQLite에 저장한다.
6. iPad 앱은 생성된 결과를 quick action detail sheet에서 보여준다.
7. Quiz study flow에서 사용자는 답안을 저장하고, 정답 확인 후 self-check를 남긴다.
8. `is_correct=false`인 `quiz_attempts`는 review-again listing/replay 대상이 된다.
9. Review-again 목록에서는 사용자가 해당 문제를 다시 풀어보거나 직접 제거할 수 있다.
10. 저장된 문장, 용어 모음, 관련 detail sheets는 `study-records` API/DTO를 사용해 document-level saved results를 조회한다.

현재 iPad UX에 포함된 동작:

- 마지막으로 열었던 문서 세션 복원
- 텍스트 선택 후 `QuickActionPanel` 기반 문장 저장, 설명, 용어, 퀴즈, 질문 action
- 좁은 화면 폭에서도 `QuickActionPanel` action row가 적절히 표시됨
- 문제 수 선택 후 사용자가 명시적으로 quiz generation을 실행하는 흐름
- 퀴즈 답안이 비어 있으면 `모름`으로 제출
- 현재 다시 보기 replay는 재풀이 결과를 새 quiz attempt로 저장하지만, `맞음` 저장 후 기존 review-again 항목을 자동으로 정리하는 동작은 아직 분리되어 있음
- Review-again listing/detail에서 사용자가 직접 항목을 제거할 수 있음

`study-records` API/DTO는 현재 iPad의 `저장된 문장`, `용어 모음`, saved sentence detail, replay/detail sheets에서 저장된 결과를 불러오는 데 사용한다.

현재 iPad의 주요 UI는 `저장된 문장`, `용어 모음`, `다시 풀 퀴즈` 중심이므로, 별도의 `학습 기록` toolbar entry나 `DocumentStudyRecordSheet`를 주요 UI로 설명하지 않는다.

## 3. Architecture boundary

| Layer | Responsibility |
| --- | --- |
| iPad 앱 | PDF import, PDFKit reading, selected text quick actions |
| FastAPI backend | PDF upload/storage, page text extraction, SQLite persistence |
| SQLite | `documents`, `highlights`, generated results, `quiz_attempts` 저장 |
| LLM provider | selected-text study actions에 대한 structured output 생성 |

Security/runtime boundary:

- iPad 앱은 backend API만 호출한다.
- iPad 앱에는 LLM API key, provider secret, provider account detail을 저장하지 않는다.
- Backend만 LLM provider를 호출한다.
- LLM provider secrets는 backend environment variables에만 둔다.
- 업로드된 PDF는 backend runtime artifacts이며 commit하지 않는다.
- Local SQLite DB, runtime logs, `.env`, `Local.xcconfig`, private PDF content는 public docs나 Git에 포함하지 않는다.

Selected-text boundary:

- LLM task input은 사용자가 선택한 text와 PDF 내부의 필요한 주변 문맥을 분리해 구성한다.
- 현재 backend는 same-page text에서 bounded context를 사용한다.
- 매 요청마다 full PDF text를 LLM provider로 보내는 architecture가 아니다.
- LLM output은 그대로 신뢰하지 않고, 정해진 schema에 맞는지 검증한 뒤 저장한다.
...
`review_cards`는 backend/API capability로 존재하지만, 별도의 review card list/detail/edit/delete UX는 현재 주요 MVP UX가 아니며 deferred다.
...
- 기본 local database는 SQLite URL인 `sqlite:///./info_drip.db`를 바라보도록 설정한다.
- Uploaded PDF storage는 backend runtime directory를 사용한다.
- 기본 LLM provider는 local development에 적합한 `fake` provider다.
- 실제 LLM provider를 쓰려면 backend environment에서 `INFODRIP_LLM_PROVIDER=openai-compatible`와 provider-specific variables를 설정한다.

iPad:

- `INFODRIP_BACKEND_BASE_URL`은 Info.plist build setting으로 주입된다.
- Simulator 기본값은 `http://127.0.0.1:8000`이다.
- 실제 iPad로 테스트 할 때는 `ios/InfoDrip/Config/Local.xcconfig.example`을 `Local.xcconfig`로 복사하고, iPad에서 접근 가능한 sanitized backend host를 설정한다.
...
실제 사용 중인 LAN/Tailscale IP, `.env` values, `Local.xcconfig` values, provider account details, API keys, private PDF filenames/content는 public artifact에 포함하지 않는다.
...
- iPad 변경은 가능한 환경에서 Xcode build로 확인하고, 주요 user flow는 실제 iPad 수동 QA로 확인한다.
